...
% ========================================================================================
\title{%
  Beyond climate: the zonal organisation of soil organic carbon transit times
}
...
\begin{figure}[H]\centering
\includegraphics[width=0.80\textwidth]{../plots/step_15b_zonality/zonality_three_panel.png}
...
\caption{\PH{[draft figure --- to refine together]}~ \textbf{Zonality of soil-carbon turnover.} (a)~Abiotic and (b)~biological modulation of $\tau$, each relative to climate, at meso (2\textdegree) scale; (c)~a discrete companion classifying each block by which non-climate domain most modulates $\tau$ beyond climate. How to read (a,b): each panel maps how much longer or shorter turnover is than climate alone would predict, expressed as the log-ratio of the full-model prediction to the climate-only prediction in each 2\textdegree{} block. Values near zero mean climate already accounts for the local turnover; departures mark where edaphic (a) or biological (b) context lengthens or shortens turnover beyond what climate explains. Red/green~=~longer turnover, blue/purple~=~shorter. In (c), each block's non-climate log-ratio modulations are weighted by their Shapley share (Fig.~\ref{fig:shapley}) and the largest is taken; the shade gives its direction (darker~=~longer, lighter~=~shorter turnover), and grey (``none, climate alone'') marks blocks where no non-climate domain shifts $\tau$ appreciably. Climate is the largest single control everywhere and forms the baseline of all three panels: a coloured block in (c) is therefore not one where climate is unimportant, but one where a particular non-climate domain most shapes the departure from the climate baseline. Soils dominate the drylands and temperate mid-latitudes, land cover the wet and boreal forests, and biology the cold, high-latitude margins.}
\label{fig:zonality}
\end{figure}
...
Total NPP & 148,714 & 0.389 & 0.319 & 6.9 & 0.269 & 36.3 & 31.6 & 21.5 & 10.6 \\
Total $-$ harvest ($h{=}0.5$) & 149,207 & 0.390 & 0.302 & 8.8 & 0.258 & 34.0 & 32.9 & 21.6 & 11.4 \\
\bottomrule
\end{tabular}
\end{table}
