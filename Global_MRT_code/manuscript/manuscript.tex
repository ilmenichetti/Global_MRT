...
% COMMENT, Lorenzo: maybe I would prefer British English standard
...
attributable, organised, and mappable. Comparing all predictor groups using a
common spatial block cross-validation procedure
%COMMENT, Lorenzo: "common spatial block" is a bit hard to read in detail
, climate explains about a third
of the predictable variance in $\ln\tau$; an order-independent (Shapley) 
%COMMENT, Lorenzo: "order-independent" in which respect? What's the meaning of "order" in this context? Shapley will be unheard of to most readers when reading the abstract.
decomposition attributes the
rest to edaphic context (soil texture, mineralogy and chemistry; 33\,\%),
...
as a distinct second axis, and marginal-effect analysis 
%COMMENT, Lorenzo: "marginal-effect analysis" is a bit generic, hard to understand what it refers to in practice here
resolves it into
ecologically interpretable, biome-dependent response shapes. Mapping the structure
recovers a coherent zonal pattern of turnover: a modern, 
%COMMENT, Lorenzo: "modern" is self aggrandizing style, we shouldn't use these kind of terms, but be more neutral, let results speak for themselves
data-intensive analysis of global turnover times
...
against six CMIP6 models our estimated structure is gentler
%COMMENT, Lorenzo: we need to be more specific, what is gentler? It is the distribution of tau over latitude, "structure" is not qualified enough
, theirs showing a
polar-to-tropical contrast in turnover roughly three times steeper and disagreeing
among themselves by an order of magnitude. We propose the zonality of soil-carbon
turnover as an interpretive framework for global SOC turnover.
\end{abstract}
...
The relevant 
%COMMENT, Lorenzo: Relevant for what?
quantity is the mean transit time $\tau$, the average time a carbon atom
...
bulk single-pool inversion of stock over input actually recovers.
%COMMENT, Lorenzo. "recover" is maybe not the right verb here, that is the quantity estimated, specifically
...
recast SOC persistence 
%COMMENT, Lorenzo: how does the word "persistence" connects on our framework? It maps directly on tau, I would say. Word is just introduced here first time without connection.
...
%COMMENT, Lorenzo: maybe this can lead to suggesting already the advantages of this framework: by treating tau as zonal, rather than just a mechanistic consequence of climate, we would include on top of climate (already taken care of by the correlation climate/soil zonality we talk about here) other ecological variables implicitly without the need to impose mechanistic assumptions. If done, should be very very short, just a hint.
...
Horizon-level data 
%COMMENT, Lorenzo: all data? This need too be a specific statement.
...
BD product.
%COMMENT, Lorenzo: is the SoilGrids product introduced before? If not, it should be introduced first (and referenced).
Second, profiles lacking measured
...
%      coefficient toward 1; "add harvest" = lower it in managed pixels. Same NPP field.
...
%   STATUS: settled for now, but reviewers will re-raise. Full reasoning + un-bibbed
%   coefficient sources (Bolinder/Saunders/Komiyama/Campioli) in memory decision file.
% =======================================================================================

%COMMENT, Lorenzo: to me it seems mainly a convention problem, from the perspective of our modeling framework it is just a choice about how we partition our variance 
...
% Comment by Aleksi: exclude peatlands: flag-not-mask. Dropping Histosol-classified observations leaves the beyond-climate gap and its spatial organisation essentially unchanged (Table~\ref{tab:peat_sensitivity}), so they are retained in the fit and flagged (not interpreted) on the maps. 
%COMMENT, Lorenzo: actually the peat exclusion is now ongoing

Soil properties were obtained from SoilGrids 250\,m v2.0
\citep{Poggio2021}, depth-weighted to 0--20~cm: 

%COMMENT, Lorenzo: the depth weighting seems described in several place, maybe there is some redundancy
...
focal 
%COMMENT, Lorenzo: "focal" is not easily understandable in this context, should be made more explicit
standard deviation of elevation, that is, the standard deviation
...
%COMMENT, Lorenzo: why the resolution choice? If not too long/complicated might be worth to back this decision
...
belongs to each group. This is not a trivial accounting exercise, 
% COMMENT, Lorenzo: this might be rephrased to be less self-assertive, no need to say we are doing something difficult, sounds like we are trying to sell
because the
...
each of these analyses in turn; here 
%COMMENT, Lorenzo: "here" where? In this section?
we only sketch the logic that links
...
skill 
%COMMENT, Lorenzo: most readers will be used to fitness rather than skill, difference should be introduced
is reported throughout as the spatially cross-validated $R^2$, which we
...
power among correlated inputs. 
%COMMENT, Lorenzo: no need to introduce the history of the method, anything after ":" seems quite redundant and not needed
...
bands (southern extratropics, 
%COMMENT, Lorenzo: "extratropics" sounds quite exotic as term, could be more plain and possibly boring
tropics, northern temperate, boreal/Arctic).
...
\emph{Peatlands.} Excluding Histosol-classified observations (about 1.3\% of
records) leaves the gap and its spatial organisation essentially unchanged
(Table~\ref{tab:peat_sensitivity}); we retain these records in the fit and flag,
rather than interpret, $\tau$ over Histosol-dominated areas on the maps.
%
...
climate-only model already reaches a relatively high $R^2=0.307$: much of that
...
%COMMENT, Aleksi: just wondering what can be the driver of spatial correlation with 16 km range?  For me this seems like quite long distance when earlier papers with soils have shown few meter spatial correlation with soils.  Is this somehow related to input estimation (is NPP map having this kind of spatial strucure?) or what is the driver that introduces spatial correlation within 16 km ? 
%NOTE, Lorenzo: comment is taken care of but should be addressed here in the note explaining how
...
covers ${\sim}$942\,000 land pixels and yields a global mean $\tau$ of 23.3~yr
...
\caption{\PH{[draft figure --- to refine together]}~ \textbf{Zonality map (symmetric).} (a)~Abiotic and (b)~biological modulation, 
%COMMENT, Aleksi: It would helpful if caption would explain interpration of this figure also for dummies? How to read the fig. ....?
%NOTE, Lorenzo: this is taken care of now but could be useful to revise the granularity of the explanations in the captions
each relative to climate, at meso (2\textdegree) scale. How to read it: each panel maps how much longer or shorter turnover is than climate alone would predict, expressed as the log-ratio of the full-model prediction to the climate-only prediction in each 2\textdegree{} block. Values near zero mean climate already accounts for the local turnover; departures mark where edaphic (a) or biological (b) context lengthens or shortens turnover beyond what climate explains. Red/green~=~longer turnover, blue/purple~=~shorter.}
\label{fig:zonality}
\end{figure}
...
attribute (Shapley 35\,\%; single-domain $R^2=0.31$). Still, these results should not lead us to 
...
overlapping signals (Sec.~\ref{sec:results_models}). 
%COMMENT, Lorenzo: here we could refer to the correlation between climate and soil zonality mentioned before, even if we do it also in the following paragraph. There could be a connection.
The beyond-climate
...
zonality that organises soil classification, could represent an interpretative tool that helps draw the mechanistic findings of the past two decades into a synthesis useful for predictions. For modeling, the consequence is concrete: turnover becomes a set of region-specific sensitivities rather than a single climate-scaled law, so a meaningful share of the spatial variance in kinetics will remain unexplained by any global parameterization. Because each process class is most expressed in a different region, such a framework would need to be spatially explicit, with region-specific combinations of climatic, edaphic, biological, and land-use modulation. The marginal-effect shapes we recover (thresholds, saturation, monotonic and
...
account for part of the divergence. It does, however, point to mid-latitude and subtropical
...
%COMMENT, Lorenzo: this work was part of the NextGenC EU project, should be acknowledged
...
\begin{figure}[H]\centering
\includegraphics[width=0.95\textwidth]{../plots/step_15b_zonality/zonality_dominant.png}
\caption{\PH{[exploratory --- candidate for promotion]}~ \textbf{Dominant non-climate modulator of $\tau$ (discrete zonation).} Each meso (2\textdegree) block is classified by which non-climate domain (edaphic, land use, or biological) most modulates $\tau$ relative to a climate-only baseline, each domain's log-ratio modulation weighted by its Shapley share of attributable variance (Fig.~\ref{fig:shapley}) so the zonation is consistent with the attribution; blocks where no non-climate domain shifts $\tau$ appreciably are labelled climate-dominated. Soils dominate the drylands and temperate mid-latitudes, land cover the wet and boreal forests, and biology the cold, high-latitude margins. This is a discrete companion to the continuous modulation field (Fig.~\ref{fig:zonality}).}
% COMMENT, Lorenzo: I would promote this into the main text as a third panel of Figure 3 (the zonality map). Should then be addressed in methods (I think section \subsection{Mapping the zonality, and choosing the scale})
%But something maybe weird of this data treatment approach: the climate-dominated regions are suspiciously little (considering climate is after all still the major explanatory group, even if by just a little), maybe the choices in this figure gives a false narrative?

\label{fig:zonality_dominant}
\end{figure}
...
\midrule
Belowground (main) & 154,675 & 0.380 & 0.307 & 7.3 & 0.252 & 35.1 & 32.8 & 22.3 & 9.7 \\
Total NPP & 152,410 & 0.384 & 0.310 & 7.4 & 0.265 & 35.5 & 32.9 & 21.8 & 9.8 \\
Total $-$ harvest ($h{=}0.5$) & 152,912 & 0.382 & 0.296 & 8.6 & 0.260 & 34.0 & 32.5 & 22.2 & 11.3 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Peatland sensitivity of the beyond-climate signal (flag-not-mask decision). Re-running the full commonality decomposition with Histosol-classified observations dropped (1,157 common rows removed, $\sim$1.3\% of the full dataset) leaves the beyond-climate gap and its spatial organisation essentially unchanged; the gap in fact decreases slightly, i.e. organic soils are not inflating the signal. Histosols are therefore retained in the fit and flagged (not interpreted) on the maps.}
\label{tab:peat_sensitivity}
\begin{tabular}{lrccccrrrr}
\toprule
 & & \multicolumn{2}{c}{Block-CV $R^2$} & Gap & Moran's & \multicolumn{4}{c}{Shapley share (\%)} \\
\cmidrule(lr){3-4}\cmidrule(lr){7-10}
Run & $n$ & full & clim. & (pp) & $I$ & C & E & B & L \\
\midrule
Main (Histosols retained) & 154,675 & 0.380 & 0.307 & 7.3 & 0.252 & 35.1 & 32.8 & 22.3 & 9.7 \\
Histosols dropped & 153,518 & 0.381 & 0.314 & 6.7 & 0.248 & 35.7 & 32.7 & 22.5 & 9.1 \\
\bottomrule
\end{tabular}
\end{table}
