...
biology all leave a mark), and soils themselves are organised zonally, their
...
common spatial block cross-validation procedure, climate explains about a third
of the predictable variance in $\ln\tau$; an order-independent (Shapley) decomposition attributes the
...
as a distinct second axis, and marginal-effect analysis resolves it into
ecologically interpretable, biome-dependent response shapes. Mapping the structure
recovers a coherent zonal pattern of turnover: a modern, data-intensive analysis
converging on the long-standing pedological insight that soils, and their emergent
properties, are zonal. Because this rests on soil observations rather than model
output, it also offers an observation-based benchmark for Earth System Models:
against six CMIP6 models our estimated structure is gentler, theirs showing a
...
bulk single-pool inversion of stock over input actually recovers. Because the total global stock is so large, even
...
thermal, a premise our beyond-climate analysis lets us examine directly.
...
g~cm$^{-3}$, and depth in cm (here the value~20, the thickness of the
...
% ============ REVIEWER POINT + DECISION: carbon input = belowground NPP only ============
% RAISED BY: Aleksi (x2: are branches left on site? is all BNPP really litter? is
...
% DECISION (belowground as MAIN, defended on principle + sensitivity, NOT just omission):
...
%   4) CIRCULARITY RULE (governs what may enter the denominator): land-cover/management-
...
% ================ REVIEWER POINT + DECISION: forest harvest removals ================
...
%   STATUS: settled for now; reviewers may push for a real management layer. See memory
%   decision file for the h anchors and the circularity reasoning.
% ===================================================================================
...
% Addressed 2026-07-07 (Aleksi: exclude peatlands): flag-not-mask. Dropping Histosol-classified observations leaves the beyond-climate gap and its spatial organisation essentially unchanged (Table~\ref{tab:peat_sensitivity}), so they are retained in the fit and flagged (not interpreted) on the maps. Decision record: manuscript/decisions/2026-07-07_peat_flag_not_mask.md.
...
% NOTE (Lorenzo): not sure we will keep the current flag-not-mask decision --
%   wants this question and the answer recorded here for now.
%
% ANSWER (this is what the flag-not-mask decision rests on): we already tested
%   exactly this. Dropping the Histosol-classified observations (2,573 sites; 1,157
%   common rows, ~1.3% of the data) does NOT distort the interpretation in the
%   feared direction. The beyond-climate gap moves 7.3 -> 6.7 pp and climate-only
%   R^2 rises 0.307 -> 0.314, i.e. peat sites if anything ADD to (do not inflate)
%   the beyond-climate signal; Moran's I is flat (0.252 -> 0.248) and the Shapley
%   shares barely move (Table~\ref{tab:peat_sensitivity}). So keeping peat in the
%   fit is conservative for our central claim, which is why we chose flag-not-mask
%   (keep in fit, do not interpret tau over organic soils on the maps) rather than
%   a full exclusion. Two caveats if we DO decide to exclude: (i) the filter is a
%   0.1-deg modal WRB Histosol class, which misses thin/degraded peat, so a global
%   peat map (e.g. PEATMAP / Xu et al. 2018) would catch more but is a new layer +
%   a full pipeline re-run + co-author re-review; (ii) the steady-state mineral-soil
%   tau framing does not apply to organic soils regardless, which is the argument
%   for flagging them on the maps in either case. Decision record:
%   manuscript/decisions/2026-07-07_peat_flag_not_mask.md. NOT YET FINAL -- revisit.
% ================================================================================
