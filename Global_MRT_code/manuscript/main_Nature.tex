...
describes is organised at large, geographic scale, as soils themselves are, has not
been asked. If persistence is emergent and soils are
zonal,

% We do know that soils are zonal, that's what pedology is based upon. We are introducing that framework in SOC decay theory, merging the two. The tension is testing what this hypothesized framework does, how does it work

the turnover that climate leaves unexplained should not be noise but zonal
geography. We test this on an existing global compilation of mineral-soil profiles,
onto which we assemble climatic, edaphic, biological and land-use covariates,
estimating steady-state transit times and attributing their variation on held-out
geographic regions. Climate accounts for about a third of the predictable variation;
the rest is coherently organised, edaphic-led with soil biology a distinct second
axis, and forms a zonal pattern of turnover. A data-intensive analysis thus returns
to the century-old pedological principle that soils, and their emergent behaviour,
are zonal, and it yields an observation-based benchmark whose poleward gradient
current Earth System Models render several times too steep.

%too rough judgement on ESM models, we don't know what is truth here. They just render it different than us, a lot steeper... reader will judge.
...
That premise inherits a simplification on which most of the theory on soil C decomposition has been based since decades. Since the foundational
...
%we should also mention that the old theory kind of works, it does capture a lot of the variance. It has survived so long for reasons (on top of being simple, and the difficulties of implementing more complex theories due to complexity and emergence, the "reductionism doesn't work in SOC decay" pitch). But not all. Some refs here would be needed.
Evidence accumulating over
the past two decades revised it, and its synthesis \citep{Schmidt2011} recast
persistence as a property of the whole soil system rather than of its molecules, set
...
therefore inherit that zonal organisation. In the present study we put to test this proposed framework, that the variation in turnover 
%ius "turnover" the correct term?
that climate does not
explain is not noise but zonal geography, the large-scale signature of persistence
%the term "persistence" can be used but must be made consistent with the whole framework. We define tau as transit time, how does it relate to persistence? Terminology on the main quantity we consider must be absolutely consistent.
as an emergent property.

We take a route complementary to mechanistic modelling.
%here could be the place for a small, 1-2 lines, digression on the challenges of mechanistically defining persistence as an emergent ecosystem process (from complexity), also creating tension. Could be earlier if it fits better.
Rather than prescribe how
...
each process class (climate, the edaphic matrix, the biological system that supplies
and processes carbon, and land use); whether that variance is organised or
scattered; where localy ecology lengthens or shortens turnover; and how each driver acts.
The beyond-climate signal we find is modest in size but coherently
% what does it mean exactly "coherently" here?
organised and
ecologically interpretable (edaphic-led, with soil biology a distinct second axis);
mapped, it forms a zonal pattern of turnover; and that pattern
...
On the set of profiles complete across all four predictor groups ($n=150{,}863$), a
...
Fig.~\ref{fig:shapley}) places climate at 35\,\%, the edaphic matrix at 33\,\%, soil
...
coupling made visible, and it is why a zonal description of SOC transit times could be efficient rather than
redundant, carrying the shared climate signal and adding organised structure
% what does "organised structure" refers to? This should be more down to the core of what we are writing of, our proposal adds the variance emerging from other ecological interactions that climate alone cannot capture
on top
of it.

\paragraph{The remainder is organised, not scattered.}
%this means signal not noise, right? Might be a good way to say it. Otherwise: organized how?
...
a continuous 
% it's not really continuous tho, we discretize. Might be fine, maybe, but it is imprecise, could be better
profile of how the beyond-climate attribution changes from equator to
...
% "Soil matrix" is hard to understand what it refers to, this throughout the manuscript. Is it edaphic haracteristics, or something like that, we refer to wqith "soil matrix"? In pedology soil matrix means soil granulometry, so it is wrong as term.
...
own spatial axis rather than tracking the soil matrix. What that axis represents is
