...
persists in soil is an emergent property of the whole ecology of soil systems. Soils themselves are
...
organised and mappable has not been established.
%the zonality of SOC decomposition hypothesis is not well articulated in the abstract. I mean the story is not expressed as arc here, there is no tension established in this first section.

Here we compile a global set of
...
what controls $\tau$, and how it varies geographically, impacts directly whether
...
That premise follows from a durable
%"durable" sounds weird here. What does it mean? That the simplification has a long history?
simplification. Since the foundational models
...
The past two decades have recast 
%"recast" is an exotic term, restructure
that view: persistence is now read as an ecosystem
...
The implication has not been followed to its spatial
conclusion. 
%this phrase could be connected more with the previous in a sub-arch
If the properties that set persistence arise locally, they should carry
...
organisation. 
% also this phrase needs to be connected more organically and with more tension, the story is not flowing well as before
Read this way the remainder that climate leaves unexplained need not
...
%we didn't compile the observation dataset, that was compiled already. We added a lot of covariates to it.

, we ask four questions of the variance beyond climate,
...
scattered; where context lengthens or shortens turnover; and how each driver acts. We find that the beyond-climate
...
%confirm the above impression, the narrative arch is a bit weak to follow here
...
order-independently
% "order-independently" needs to be explained, what does it mean in this context? Which order, and of what?
among the four groups (Shapley decomposition; Methods,
...
importance ranking), 
% here a connection with the main narrative arch. That premise we are examining holds but only to some extent, which is revealed in what follows.
yet it accounts for only about a third of the variance the
model can attribute; the non-climate classes carry the remaining two thirds. The
single-domain skills
%"single domain skills" is hard to read at this point, also not having defined "skill"
tell the same story from the other side: edaphic predictors on
...
%this phrase could be modified to express that, because of that, our proposed framework is quite competitive since it does still explain that climate variance and something else on top (not necessarily here)
...
this holds across climate zones rather than in one. 
%"rather than in one" is not working so much. Does it mean "just in one"? Maybe this formulation can be revised and expressed more directly
The organised part
% "part" is a bit generic word here
is a
...
Refitting the decomposition
%composition of what?
within contiguous latitude bands (Methods; Extended Data
Fig.~\ref{edfig:latshapley})
%what does it mean procedurally, to refit with contigous bands?
turns the discrete zonation into a quantitative profile
%profile over what?
...
variance at 50--60\textdegree N. 
% this is a pretty cool result and will have later to be connected well with literature. I see later it is done already.
Land use is a modest contributor that never leads
%too diminishing for land use, we know it is important regardless because of inputs, although yes, these are the results. Controversial statement, better to avoid given the little robustness of a global study (we will never have a very good robustness with these data, we are hinting)
, roughly steady through the low and middle latitudes and falling to near zero in the
boreal. Because each band's decomposition removes the poleward gradient
% hard to read this phrase above, what does it mean? what is happening?
, these shares
describe the spatial structure within a belt rather than the level set across belts
%also a bit obscure statement, needs to be anchored more. Which "belt"?
,
...
Although the edaphic and biological modulation fields look broadly similar
% in which respect?
, the
...
the climate-plus-edaphic model, its increment is essentially uncorrelated 
%what correlation we talk about? Spatial?
with the
abiotic field ($r\approx0$; Methods), so soil biology forms its own spatial axis.
The identity of that axis 
%what's the "identity of an axis"?
is suggestive
% "suggestive" sounds too poetic
rather than settled
% "settled" what does it refers to? Literature? Here we will have to expand, use placeholders but there is going to be a heavy literature work
: mycorrhizal
...
independent grounds
%qualify "independent grounds", too generic wording
. We flag it as an interesting possibility to test directly, not
...
readable: coarse (sandy) texture shortens turnover (weaker organo-mineral
stabilisation), rising pH lengthens it to a peak near 7.5--8, snow cover lengthens
it monotonically, and a higher fungal proportion shortens it. 
% these are speculations. Which makes sense, but still speculation, we don't have the data to say it is for sure like this. It just might be, it does fit the current hypotheses (and everyone of these statements must be ground in literature, use placeholders)
Several shapes are not
monotonic, and (more tellingly)
% why? Telling what?
several vary by latitude band rather than following
...
% "fits" is a unhappy terms that suggests we are trying to make it fit, reformulate the title of this paragraph more neutrally
...
%this whole paragraph is controversial. Management has for sure huge importance locally, since it conditions C inputs to the soil which are always the main driver. Just at this scale that crucial contribution is lost, management is likely much more localised even if it will carry more signal. Scale is crucial to read all our results.
...
climate. 
%here there should be tension resolution, in this paragraph. We are implementing the concept of SOC decay as emergent property, which was controversial and impacting 10 years ago, at a global scale, this can definitely be used to build tension and release it
The beyond-climate signal is modest in absolute predictive terms (about
...
response shapes.
% another minor resolution is that we are quantifying the impact of that framework, ther emergenct properties thing
Treating the zonality of soil-carbon turnover as a working
