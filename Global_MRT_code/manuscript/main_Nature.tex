...
built bottom-up from small scale processes inside the soil, and never asked whether it is organised
...
this makes soil carbon a linear (first-order) non-autonomous system: the
...
climate. The description has resisted for so long 
...
individual profiles most of the variation is short-range: about 56\,\% of the
...
observed residual roughly doubles from the native pixel ($R^2\approx0.11$) to
$\approx0.22$ at blocks of 1--2\textdegree{} before plateauing, which is why we map
...
uncorrelated with the abiotic one ($r\approx0$ between the two maps; Methods), so
...
following a single global curve: the temperature-seasonality response is strongest
in the boreal/Arctic zone and near-zero in the tropics, and the pH response is
steepest in northern soils, whereas the biological responses are comparatively
uniform across bands (Extended Data Fig.~\ref{edfig:ale_latband}). That latitude
dependence is one of the clearest results of this analysis: turnover behaves as a
collection of region-specific sensitivities rather than as a single climate-scaled
law. The functions we recover indicate what biome-specific formulations might look
like; they are not ready-made ones.
...
up as variance between databases, and only 2.4\,\% of the residual variance does
...
only to $\approx16$~km). Enlarging the block to $1^{\circ}$ or $5^{\circ}$ lowers absolute
