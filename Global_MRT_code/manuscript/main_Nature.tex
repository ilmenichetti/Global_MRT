...
% NOTE, Lorenzo: maybe it could talk a bit more openly of the advantage of the framework we propose of not having to consider emergent properties in a reductionist way (which is almost by definition not a good idea)

\paragraph{Spatial patterns of soil C persistence.}
The reading of soil C persistence as an emergent property, until now discussed from the perspective of soil's internal processes
\citep{Schmidt2011}, is here made spatial. Across the globe,
the persistence that climate does not set is not scattered but organised, and organised
zonally (Moran's $I=0.25$, edaphic-led with soil biology a distinct axis); SOC persistemce carries large-scale spatial structure, and that structure can be connected with the pedological
zonality. The beyond-climate signal is modest (about seven percentage points of
variance) but real, ecological rather than an artefact of data provenance (only
2.4\,\% of the residual variance lies between source databases; Methods), and readable
through biome-dependent response shapes. Treating that zonality as a working framework
draws the findings of the past two decades regarding SOC decomposition into a form usable for
prediction.
